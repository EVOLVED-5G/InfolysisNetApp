      ...
      <section class="content-header">
        <div class="col-md-12">
          <?php 
            ...
            $sql_machine_area = "SELECT area from machines where id=".htmlspecialchars($_GET["machine_id"]);
            $result_machine_area = m_execute_query($con, $sql_machine_area);
            $row_machine_area = mysqli_fetch_row($result_machine_area);
            $machine_area = $row_machine_area[0];
            $sql = "SELECT * FROM machines WHERE id = ".htmlspecialchars($_GET["machine_id"]);
            $result = m_execute_query($con, $sql);
            $row = mysqli_fetch_array($result);
            echo "<h1>
            ...
            <div class="box">
              <div id="user_selection">
                <nav class="navbar navbar-default">
                    <ul class="nav navbar-nav">
                      <?php
                      echo '<li class="active" id="machine_info"><a href="machine_info.php?machine_id='.htmlspecialchars($_GET["machine_id"]).'">Info<i class="fa fa-fw fa-info-circle"></i><span class="sr-only">(current)</span></a></li>';
                      echo '<li id="machine_incidents"><a href="machine_incidents.php?machine_id='.htmlspecialchars($_GET["machine_id"]).'">Incidents<i class="fa fa-fw fa-wrench"></i></a></li>';
                      echo '<li id="machine_files"><a href="machine_files.php?machine_id='.htmlspecialchars($_GET["machine_id"]).'">Files<i class="fa fa-fw fa-book"></i></a></li>';
                      ...
                      <div class="card-body">
                          <form  role="form" method="post" id="machine_form">
                            <div class="form-group">                              
                              <label>Name</label>
                        <?php echo "<input type='text' name='machine_name' id='machine_name_id' class='form-control' placeholder='Enter Name...' value='".$row["name"]."'>";?>
                            </div>
                            <!-- <div class="form-group">
                              <label>Area</label>
                  <?php echo "<input type='text' name='machine_area' name='machine_area_id' class='form-control' placeholder='Enter Area...' value='".$row["area"]."'>";?>
                            </div> -->
                            <div class="form-group">
                            ...
                            <div class="form-group">
                              ...
                              <select id="machine_status_id" class="form-control" name="machine_status">
                                <option value="-1">Choose...</option>
                                <option value="1" <?php if ($row["status"]=="1") echo "selected";?>>1</option>
                                <option value="2" <?php if ($row["status"]=="2") echo "selected";?>>2</option>
                                <option value="3" <?php if ($row["status"]=="3") echo "selected";?>>3</option>
                                <option value="4" <?php if ($row["status"]=="4") echo "selected";?>>4</option>
                              </select>
                            </div>
                          ...
                          <div class="col-sm-2 col-sm-offset-2" id="cancel_button">
                            ...
                            <div class="col-sm-2 col-sm-offset-4" id="update_button">
                              <button type="button" class="btn btn-block btn-success" id="machine_info_update"><i class="fa fa-fw fa-save"></i>Update</button>
                            </div>
                      </div>
                      <!-- /.card-body -->
                    </div>  
